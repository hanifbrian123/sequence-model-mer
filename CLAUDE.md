...
- Shell PowerShell / Git Bash, GPU RTX 3060 (~9,7 GB VRAM bebas).
- **Semua hasil masuk ke `results/results.csv`** (satu-satunya tabel angka).
- **Semua laporan masuk ke `reports/LAPORAN.md`** (satu-satunya file laporan). Narasi disunting di `docs/narasi.md`.
- **Tidak boleh ada file script `.sh`, `.csv`, `.log`, atau `.py` lepas di root** — dijaga oleh `tests/test_structure.py`.
- Sebelum menyerahkan pekerjaan, jalankan: `bash scripts/check.sh` (wajib hijau).

## Struktur Repositori (Standar `fer/`)

```
configs/              NNN_nama.json (bernomor 3-digit berurutan kronologis)
configs/protocols/    definisi fold/split evaluasi (accuracy_v2.json, dll)
runs/                 NNN_nama/ (folder run asli berisi summary.json, run.log, probs.npz)
results/              results.csv (SATU tabel untuk semua angka)
reports/              LAPORAN.md (SATU dokumen resmi laporan seluruh eksperimen, hasil generate otomatis)
docs/                 narasi.md, context.md, handoff, panduan teknis, thought process
scripts/              semua otomasi bash (run_queue.sh, check.sh, rebuild_reports.sh, run_loso26.sh)
src/casme/            paket Python utama (data, models, training, evaluation, reporting, serving)
src/_legacy/          arsip file Python lama dari src/ root (referensi, tidak dipakai aktif)
logs/                 log antrean sekali pakai (gitignored)
deploy/               app_predict.py dan skrip deployment
models/               model final untuk deployment
cache/                cache preprocessing (flow, frames, masks)
eda/                  exploratory data analysis
tests/                test_structure.py (penjaga struktur) + regression tests
```
...
| Perintah | Fungsi |
|---|---|
| `bash scripts/run_queue.sh <prefix\|nama\|all>` | Jalankan antrean run (otomatis lewati yang sudah selesai, susun laporan, dan tes struktur) |
| `bash scripts/rebuild_reports.sh` | Bangun ulang `results/results.csv` + `reports/LAPORAN.md` tanpa training |
| `bash scripts/check.sh` | Jalankan tes unit dan tes struktur |
| `bash scripts/run_loso26.sh` | Eksekusi benchmark LOSO 26 |
| `python -m casme.reporting.collect` | Kumpulkan semua summary run ke `results/results.csv` |
| `python -m casme.reporting.report` | Generate master `reports/LAPORAN.md` |
...
- `091_r3d_auto_apex_s42` **selesai penuh 4 fold** (`complete: true`, 20:20:38),
...
1. `eval_rand_start` tidak aktif di jalur TTA (TTA memanggil dataset dengan
   `train=True`) — akibatnya hasil `056_r3d_diff_evalshift` dulu keliru.
...
| Automatic-apex label-free (`091_r3d_auto_apex_s42`) | 0,6707 | 0,7008 | 0,6719 | selesai 4 fold |
| Energy cap 65% (`093_r3d_auto_apex_cap65_s42`) | 0,6574 | 0,6891 | 0,6563 | **ditolak** |
| Multi-hipotesis 6 view (`094_r3d_multihyp_apex_s42`) | 0,6654 | 0,7070 | 0,6563 | **ditolak** |
| Multi-hipotesis 15 view (`095_r3d_multihyp15_apex_s42`) | 0,6781 | 0,7091 | 0,6719 | **ditolak** — lihat catatan fusi |
| Oracle reuse dari snapshot (`092_r3d_oracle_apex_reuse_s42`) | 0,6995 | 0,7088 | 0,6927 | kontrol: reproduksi `066` persis |
...
Leaf `066_r3d_onset_apex_corrected` dievaluasi memakai **apex beranotasi** dari dataset — tidak ada di
video upload. Penggantinya yang label-free (`096_fusion_deployable_50full_50auto47`)
...
- **Seluruh keluarga stabilisasi gerakan.** Dua pendekatan independen sama-sama
  jatuh ~0,09 UF1: kompensasi translasi global (`040_flow_comp`, 0,5987) dan
  HQ TV-L1 + ECC euclidean (`041_hq_stab`, 0,5906) di atas `cache/flow144_hq_stab`
...
- **HQ TV-L1 + ECC** (`041_hq_stab`, 0,5906) dan **face-ellipse ROI** (`049_face_roi`,
  0,6370), keduanya jauh di bawah baseline 0,6816.
- **Apex smoothing** radius 2/4/6/10 (`053_smooth*`), semuanya ≤ radius 0.
...
- **Skor leaf tidak memprediksi nilainya dalam fusi.** `048b` menang sendirian atas
  `047_auto_apex` (0,6781 vs 0,6707) tapi fusinya justru jatuh (0,6789 vs 0,7021,
...
Semua dari snapshot `047_r3d_auto_apex_s42`, tanpa training ulang.
...
`src/casme/evaluation/eval_fps_robustness.py`:

| fps | 200 | 120 | 60 | 30 |
|---|---|---|---|---|
| leaf auto-apex (`047_auto_apex`) | 0,6707 | 0,6866 | 0,7113 | 0,6537 |
...
(`src/_legacy/dataset.py:133`) — keduanya `EVAL_ONLY_KEYS`, jadi cukup
